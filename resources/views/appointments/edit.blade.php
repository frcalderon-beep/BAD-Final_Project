...
<div class="max-w-2xl mx-auto">
    <h1 class="text-4xl font-bold text-gray-800 mb-6">Edit Appointment</h1>
    ...
    <div class="bg-white rounded-lg shadow-md p-8">
        <form action="{{ route('appointments.update', $appointment) }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <select id="system_id" name="system_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500 @error('system_id') border-red-500 @enderror">
                    @foreach ($systems as $system)
                        <option value="{{ $system->id }}" {{ old('system_id', $appointment->system_id) == $system->id ? 'selected' : '' }}>System {{ $system->id }}</option>
                    @endforeach
                </select>
                @error('system_id')
            ...
            <div class="mb-6">
                ...
                <select id="staff_id" name="staff_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500 @error('staff_id') border-red-500 @enderror">
                    @foreach ($staff as $member)
                        <option value="{{ $member->id }}" {{ old('staff_id', $appointment->staff_id) == $member->id ? 'selected' : '' }}>{{ $member->name }}</option>
                    @endforeach
                </select>
                @error('staff_id')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            ...
            <div class="mb-6">
                ...
                <select id="service_type_id" name="service_type_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500 @error('service_type_id') border-red-500 @enderror">
                    @foreach ($serviceTypes as $service)
                        <option value="{{ $service->id }}" {{ old('service_type_id', $appointment->service_type_id) == $service->id ? 'selected' : '' }}>{{ $service->service_name }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-6">
                ...
                <input type="text" id="service_name" name="service_name" value="{{ old('service_name', $appointment->service_name) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500 @error('service_name') border-red-500 @enderror">
                @error('service_name')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <input type="datetime-local" id="appointment_date_time" name="appointment_date_time" value="{{ old('appointment_date_time', $appointment->appointment_date_time->format('Y-m-d\TH:i')) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500 @error('appointment_date_time') border-red-500 @enderror">
            ...
            <div class="mb-6">
                ...
                <textarea id="personal_references" name="personal_references" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500">{{ old('personal_references', $appointment->personal_references) }}</textarea>
            </div>

            <div class="mb-6">
                <label for="notes" class="block text-sm font-semibold text-gray-700 mb-2">Notes</label>
                <textarea id="notes" name="notes" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500">{{ old('notes', $appointment->notes) }}</textarea>
            </div>

            <div class="flex gap-4">
                <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700">Update Appointment</button>
                <a href="{{ route('appointments.show', $appointment) }}" class="bg-gray-500 text-white px-6 py-2 rounded hover:bg-gray-600">Cancel</a>
            </div>
        </form>
    </div>
</div>
